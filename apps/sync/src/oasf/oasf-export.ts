...
function buildNameIndex(items: OasfEntity[]): Map<string, string> {
  ...
  for (const item of items) {
    if (!map.has(item.name)) map.set(item.name, item.key);
  }
  return map;
}
...
export async function exportOasfTtlToString(): Promise<{
  ttl: string;
  skillCount: number;
  domainCount: number;
  repo: string;
  ...
  for (const skill of skills) writeEntityLines(lines, skill, 'skill', skillNameIndex);
  for (const domain of domains) writeEntityLines(lines, domain, 'domain', domainNameIndex);

  return { ttl: `${lines.join('\n')}\n`, skillCount: skills.length, domainCount: domains.length, repo, ref };
}
