## ERC-8004 ontology (`apps/ontology/ontology/erc8004.ttl`)

Source: `apps/ontology/ontology/erc8004.ttl`

### Smart agents (SmartAccount-backed agents)

- **`erc8004:AIAgent8004`**: base ERC-8004 agent specialization (subclass of `core:AIAgent`)
- **`erc8004:SmartAgent`**: ERC-8004 agent that has an associated smart account
- **`erc8004:hasAgentAccount`**: links `erc8004:SmartAgent` → `erc8004:AgentAccount` (also an `eth:Account`, typed later as smart-vs-EOA)

On-chain NFT metadata (from the subgraph `agentMetadata_collection`) is captured on the **8004 identity descriptor** as:

- **`erc8004:onchainMetadataJson`**: lossless JSON capture of the KV set (decoded best-effort)
- **`erc8004:registeredBy`**, **`erc8004:registryNamespace`**: best-effort extracted fields
...
`erc8004:AgentRegistration8004` is an ERC-8004 agent registration descriptor that extends `core:AgentDescriptor`. It contains all agent registration data extracted from agentURI registration JSON (rawJson metadata), including:
...
**Query ERC-8004 identity descriptor + assembled protocol descriptors:**
```sparql
...
SELECT ?agent
       ?did8004
       (xsd:integer(REPLACE(STR(?did8004), "^did:8004:[0-9]+:", "")) AS ?agentId8004)
       ?identity8004
       ?identityDescriptor
       ?registrationJson
       ?protocolDescriptor ?serviceUrl
       ?skill ?domain
...
WHERE {
  ...
  OPTIONAL { ?identityDescriptor core:descriptorName ?name . }
  OPTIONAL { ?identityDescriptor core:descriptorDescription ?description . }
  OPTIONAL { ?identityDescriptor core:descriptorImage ?image . }
  ...
  OPTIONAL { ?identityDescriptor core:hasSkill ?skill . }
  OPTIONAL { ?identityDescriptor core:hasDomain ?domain . }
}
