...
- **AssertionAct** (`agentictrust:AssertionAct`) is a **prov:Activity** (an act that asserts a situation).
- **AttestedAssertion** (`agentictrust:AttestedAssertion`) is a **prov:Entity** (a durable record/artifact generated by an Attestation/AssertionAct).
...
WHERE {
  ?assertionAct a agentictrust:AssertionAct .
