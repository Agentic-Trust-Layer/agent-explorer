...
- `erc8092:AccountAssociation8092` is the on-chain association record, modeled as an assertion activity.
- It asserts a `agentictrust:RelationshipTrustSituation` via `agentictrust:assertsSituation`.
...
AccountAssociation --> Account : initiator
AccountAssociation --> Account : approver
...
**Query AccountAssociationRevocation8092:**
```sparql
PREFIX erc8092: <https://www.agentictrust.io/ontology/ERC8092#>
...
WHERE {
  ...
  OPTIONAL { ?revocation erc8092:revocationOfAccountAssociation ?revokedAssociation . }
