...
WHERE {
  ?agent a core:AIAgent ;
    core:agentId "3223" ;
    eth:hasAccountIdentifier ?acctId .

  ?account eth:hasIdentifier ?acctId .
...
WHERE {
  ...
  ?situation a ?situationType ;
    core:isAboutAgent ?account .

  ?situationType rdfs:subClassOf* core:RelationshipSituation .
...
WHERE {
  ...
  FILTER NOT EXISTS {
    ...
    ?moreSpecific rdfs:subClassOf* core:RelationshipSituation .
  }
}
...
WHERE {
  ...
  OPTIONAL {
    ?act core:generatedAssertionRecord ?assertion ;
...
WHERE {
  ...
  OPTIONAL {
    ?act core:generatedAssertionRecord ?association ;
      core:assertsSituation ?situation .
  }
...
WHERE {
  ...
  ?agentAccount eth:hasIdentifier ?acctId .
